    ...
    <div class="add-product mt-4 ">
      ...
      <div class="btn-fixed p-3 position-fixed text-right bg-white">
        <div class="box-btn-fixed">
          <b-button type="submit" variant="primary" @click="onEdit(idProduct)">Edit</b-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    getDaata () {
      ...
      this.form.productLength = data.productLength
      this.form.productWidth = data.productWidth
      this.form.productHeight = data.productHeight
      this.form.productWeight = data.productWeight
      this.form.uomId = data.uomId
      this.form.brandId = data.brandId
      this.form.categoryId = data.categoryId
      this.form.subCategory1 = 1
      this.form.subCategory2 = 2
      this.form.productDescription = data.productDescription
      this.idProduct = data._id
    },
    onEdit (id) {
      this.$store.dispatch('product/editProduct', this.form, id)
      .then((res) => {
        if (res.success) {
          this.$swal({
